...
var _shared2 = require("@varlet/shared");
...
var _default = (0, _vue.defineComponent)({
  ...
  setup(props) {
    ...
    var parseFormat = (format, time) => {
      ...
      scannedFormats.forEach((scannedFormat, index) => {
        if (!format.includes(scannedFormat)) {
          ...
          format = format.replace(scannedFormat, (0, _shared.padStart)("" + scannedTimes[index], 2, '0'));
        }
      });

      if (format.includes('S')) {
        var ms = (0, _shared.padStart)("" + scannedTimes[scannedTimes.length - 1], 3, '0');
    ...
    var countdown = () => {
      ...
      var now = Date.now();
      if (!endTime.value) endTime.value = now + (0, _shared2.toNumber)(time);
    ...
    var start = () => {
      ...
      isStart.value = true;
      endTime.value = Date.now() + (pauseTime.value || (0, _shared2.toNumber)(props.time));
      countdown();
    }; // expose
